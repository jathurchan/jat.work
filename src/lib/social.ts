// Social profile links + icon markup, used by the footer.
import { profile } from './site';

// Inner SVG markup for each icon (filled, currentColor — viewBox 0 0 24 24).
...
// External profiles only: the email address already owns the footer's pill +
// copy row, so a third mailto icon right below it would just repeat it.
