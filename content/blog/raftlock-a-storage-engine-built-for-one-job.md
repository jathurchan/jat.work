...
RocksDB solves a harder problem than I have. Matching the engine to the access pattern gave me something simpler, faster, and far more predictable.
...
3. **Targeted performance.** Fast appends and `O(log n)` indexed reads, tuned to Raft's real workload.

## Surviving a crash mid-write

This is where most of the effort went, because it's the part that's easy to get subtly, silently wrong. Durability comes from three layered safeguards:
...
- Reads binary-search the map and seek straight to the right byte, in `O(log n)`.
