...
**A deterministic simulation framework** driving a virtual clock and a simulated network, so I can orchestrate and *reproduce* thousands of randomized failure scenarios:
...
After every run it checks Raft's safety invariants (no two leaders in one term, no committed entry ever overwritten) and fails loudly if any are violated. Every failure is reproducible from its seed, so a bug found at 2am is a bug I can re-run on demand. This Jepsen-style approach is the single biggest reason I'll vouch for the implementation under real-world chaos.
...
- **Batching for efficiency.** Proposals are batched before replication, which buys better network and disk efficiency at slightly higher per-request latency. Configurable, so operators can tune it to their workload.
